class Entity:
    ...
    def __str__(self):
        companies_str = "\n".join([f"  {idx}. {company.social_reason} - CNPJ: {company.cnpj}" for idx, company in enumerate(self.companies, start=1)])
        return (
            f"╔════════════════════════════════════════════════════╗\n"
            f"║ Minha Empresa: {self.social_reason}\n"
            f"║ CNPJ:          {self.cnpj}\n"
